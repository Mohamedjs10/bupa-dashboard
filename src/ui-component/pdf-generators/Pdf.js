...
const PdfViewer = ({ uploadedEnCertificate }) => {
  ...
  console.log(translatedArCertificate);
  ...
  const convertToPDF = (option) => {
    ...
    const methodToCallParam = option === 3 ? 'dataurlnewwindow' : 'arabic-certificate.pdf';
    ...
    //       // console.log('gggggggggggggggggggggggggggggggggggg:', url);
    ...
    if (contentRef.current) {
      html2pdf()
        ...
        .then((doc) => {
          console.log(doc);
          var pdfData = doc.output('blob'); // Get PDF data as a Blob

          // Create a URL for the Blob object
          var url = URL.createObjectURL(pdfData);
          console.log(url);
          setTranslatedArCertificate(`${url}`);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      {pageContent === 2 && (
        <Box>
          <iframe title="PDF Viewer" src={url} width={`${2480 / 4}px`} height={`${3508 / 4}px`} frameBorder="0" />
